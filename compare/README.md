...
| `make compare-report` | Markdown **size table**: **`qf_math.o`** (float TU), libfixmath subset (**fixed**, bench-linked `.o`s), **`FR_math.o`** (**fixed**). Rows describe **different domains**, not one bundle. |
...
7. **`examples/lilygo_t_display_s3_bench/`** — **PlatformIO** for **LilyGO T-Display-S3** (`lilygo-t-display-s3`): **`pio run -t upload -t monitor`** after **`make compare-deps`**. This firmware appends ESP32-S3 fast-math peer rows for **FastTrig**, **ESP-DSP** sqrt, and **espp/math** scalar helpers. See **[examples/lilygo_t_display_s3_bench/README.md](../examples/lilygo_t_display_s3_bench/README.md)**.
...
- **Coverage**: generated timing/error tables currently cover **`sin`**, **`cos`**, **`sqrt`**, **`ln`**, and **`exp`** only. Additional API accuracy (`tan`, inverse trig, `log2`, `log10`, `pow2`, `pow10`, `hypot`, etc.) should be added as a separate generated coverage table before mixing it with platform timing.
