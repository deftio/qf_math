...
| `make docker-sizes` | Cross-compile object-size matrix in Docker, write `build/docker_sizes.csv`, and refresh the compact summary below. |
...
| **qf_math (lean core)** | `build/qf_math_lean.o` from **`make lib-lean`** | Same TU with **`-DQF_MATH_LEAN`**: radian trig, inverse trig, `log2`/`ln`, `pow2`/`exp`/`pow`, `sqrt`, and `hypot_fast8` — **no** degree/BAM trig entry points, exact `hypot`, `hypot_fast2`, `log10`/`pow10`, waves, or ADSR. Use this row for a minimal float-core ROM comparison. |
...
|--------|--------:|--------:|--------:|------------------:|
| Pico 2 ARM-M33 (hard-float) | 6.6 KB (6760 B) | 7.8 KB (7940 B) | 4.4 KB (4504 B) | 1.50x |
| RP2040 (Cortex-M0+) | 7.0 KB (7172 B) | 8.3 KB (8456 B) | 4.6 KB (4704 B) | 1.52x |
| ESP32/S3 (Xtensa esp-elf) | 7.0 KB (7189 B) | 8.3 KB (8514 B) | 5.4 KB (5492 B) | 1.31x |
| RISC-V 32 (rv32im) | 10.2 KB (10464 B) | 12.9 KB (13236 B) | 7.0 KB (7196 B) | 1.45x |
| x86-64 | 7.5 KB (7650 B) | 9.2 KB (9408 B) | 6.2 KB (6302 B) | 1.21x |

`qf lean` is `-DQF_MATH_LEAN`; `fr core` is `-DFR_CORE_ONLY`. These are object-file totals only, not final firmware flash.

<!-- END_DOCKER_SIZE_SUMMARY -->
...
6. **`QF_MATH_ARCH_SPEED.md`** — qf-vs-**`libm`** speed ratios by architecture (see file). Regenerate **`make benchmark-arch-speed`** after snapshot updates.
7. Run **`make docker-sizes`** when you need fresh apples-to-apples object sizes. The full matrix is `build/docker_sizes.csv`; only a compact summary is committed here.
8. Run **`make compare`** / **`make compare-report`** locally when you need numbers for *your* compiler / ISA / OS.
9. **`examples/lilygo_t_display_s3_bench/`** — **PlatformIO** for **LilyGO T-Display-S3** (`lilygo-t-display-s3`): **`pio run -t upload -t monitor`** after **`make compare-deps`**. This firmware appends ESP32-S3 fast-math peer rows for **FastTrig**, **ESP-DSP** sqrt, and **espp/math** scalar helpers, plus inverse trig rows for float-domain peers. See **[examples/lilygo_t_display_s3_bench/README.md](../examples/lilygo_t_display_s3_bench/README.md)**.
10. **`examples/esp32s3_benchmark/`** — **ESP-IDF** firmware (**ESP32-S3**) that runs **[`benchmark_core.c`](benchmark_core.c)** on-chip (`esp_timer_get_time()`); requires **`make compare-deps`** first. See **[examples/esp32s3_benchmark/README.md](../examples/esp32s3_benchmark/README.md)**.
11. **`examples/pico2_benchmark/`** — **Arduino** sketch for **Raspberry Pi Pico 2 / Pico 2 W** (RP2350 ARM Cortex-M33 or Hazard3 RISC-V); same `benchmark_core.c` kernels via `micros()`. See **[examples/pico2_benchmark/](../examples/pico2_benchmark/)**.
12. **`MCU_BENCHMARK_SNAPSHOT*.md`** — committed **on-device** tables (ESP32-S3, Pico 2 ARM, Pico 2 RISC-V); refresh with **`make mcu-benchmark-snapshot`** (UART script). Firmware also prints `DOC_TABLE_*` markers for manual capture.

## Caveats

- **Inverse trig vs `-lm` (`asin` … `atan2`):** timings pair **`asinf` / `acosf` / `atanf` / `atan2f`** with **`qf_asin`** … on the same **`float`** grids (**`benchmark_core.c`**). **`asin`/`acos`** and **`atan`/`atan2`** use **different** codepaths in **`qf_math`**, and **`-lm` is not one thing**: **Darwin arm64** often ships very fast inverse-trig **`f`** stubs; another toolchain’s **`newlib`**/`picolibc` may trade speed for size differently on **Xtensa**, **ARM-M33**, and **RISC-V**. So rows can invert **between columns** or **within the four functions** without a benchmarking mistake.
...
- **Cross-target size**: `docker-sizes` is the fairer ROM-footprint view when comparing `qf_math` and `fr_math`; it uses the same container, target compiler, optimization level, and source-TU style for both libraries. The CSV is the complete source of truth.
