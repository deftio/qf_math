...
| Directory | Purpose |
|-----------|---------|
| [`quickstart/`](quickstart/README.md) | **Minimal C example** — demonstrates trig, log/exp, sqrt/hypot, waveforms, ADSR, and utility macros. `make && ./quickstart`. |
| [`quickstart_arduino/`](quickstart_arduino/) | **Arduino sketch** — same demo as `quickstart/` but using `setup()`/`loop()` and `Serial`. Open Serial Monitor at 115200. |
