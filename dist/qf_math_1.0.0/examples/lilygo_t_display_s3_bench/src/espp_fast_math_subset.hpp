/*
 * Minimal ESP32 benchmark subset of esp-cpp/espp `components/math/include/fast_math.hpp`.
 * Upstream project: https://github.com/esp-cpp/espp (MIT).
 *
 * Kept local because the full espp repository pulls a very large component/submodule
 * tree when used as a PlatformIO dependency; this benchmark only needs these scalar
 * fast-math helpers.
 */
#pragma once

#include <cstdint>
#include <cstring>

extern "C" {
#include "qf_math.h"
}

namespace espp {

static inline float fast_inv_sqrt(float value)
{
    uint32_t bits;
    std::memcpy(&bits, &value, sizeof(bits));
    bits = 0x5f3759dfu - (bits >> 1);

    float y;
    std::memcpy(&y, &bits, sizeof(y));
    return y * (1.5f - (0.5f * value * y * y));
}

static inline int round(float x)
{
    return (x > 0.0f) ? (int)(x + 0.5f) : (int)(x - 0.5f);
}

static inline float fast_ln(float x)
{
    uint32_t bx = 0;
    std::memcpy(&bx, &x, sizeof(bx));
    uint32_t ex = bx >> 23;
    int      t  = (int)ex - 127;
    bx = 1065353216u | (bx & 8388607u);
    std::memcpy(&x, &bx, sizeof(x));
    return -1.49278f + (2.11263f + (-0.729104f + 0.10969f * x) * x) * x + 0.6931471806f * (float)t;
}

static constexpr int sine_array[200] = {
    0, 79, 158, 237, 316, 395, 473, 552, 631, 710, 789, 867, 946, 1024, 1103, 1181,
    1260, 1338, 1416, 1494, 1572, 1650, 1728, 1806, 1883, 1961, 2038, 2115, 2192, 2269, 2346, 2423,
    2499, 2575, 2652, 2728, 2804, 2879, 2955, 3030, 3105, 3180, 3255, 3329, 3404, 3478, 3552, 3625,
    3699, 3772, 3845, 3918, 3990, 4063, 4135, 4206, 4278, 4349, 4420, 4491, 4561, 4631, 4701, 4770,
    4840, 4909, 4977, 5046, 5113, 5181, 5249, 5316, 5382, 5449, 5515, 5580, 5646, 5711, 5775, 5839,
    5903, 5967, 6030, 6093, 6155, 6217, 6279, 6340, 6401, 6461, 6521, 6581, 6640, 6699, 6758, 6815,
    6873, 6930, 6987, 7043, 7099, 7154, 7209, 7264, 7318, 7371, 7424, 7477, 7529, 7581, 7632, 7683,
    7733, 7783, 7832, 7881, 7930, 7977, 8025, 8072, 8118, 8164, 8209, 8254, 8298, 8342, 8385, 8428,
    8470, 8512, 8553, 8594, 8634, 8673, 8712, 8751, 8789, 8826, 8863, 8899, 8935, 8970, 9005, 9039,
    9072, 9105, 9138, 9169, 9201, 9231, 9261, 9291, 9320, 9348, 9376, 9403, 9429, 9455, 9481, 9506,
    9530, 9554, 9577, 9599, 9621, 9642, 9663, 9683, 9702, 9721, 9739, 9757, 9774, 9790, 9806, 9821,
    9836, 9850, 9863, 9876, 9888, 9899, 9910, 9920, 9930, 9939, 9947, 9955, 9962, 9969, 9975, 9980,
    9985, 9989, 9992, 9995, 9997, 9999, 10000, 10000};

static inline float fast_sin(float angle)
{
    if (angle < QF_HALF_PI)
        return 0.0001f * (float)sine_array[round(126.6873f * angle)];
    if (angle < QF_PI)
        return 0.0001f * (float)sine_array[398 - round(126.6873f * angle)];
    if (angle < (3.0f * QF_HALF_PI))
        return -0.0001f * (float)sine_array[round(126.6873f * angle) - 398];
    return -0.0001f * (float)sine_array[796 - round(126.6873f * angle)];
}

static inline float fast_cos(float angle)
{
    float a_sin = angle + QF_HALF_PI;
    a_sin = (a_sin > QF_TWO_PI) ? (a_sin - QF_TWO_PI) : a_sin;
    return fast_sin(a_sin);
}

} // namespace espp
